...
describe('Starting tests for trustit-app', function() {
  ...
  it('website title should be trustit-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('trustit-app');
    })
  });

  it('network-name should be trustit@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trustit@0.0.1.bna');
    });
  });

  it('navbar-brand should be trustit-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trustit-app');
    });
  });
